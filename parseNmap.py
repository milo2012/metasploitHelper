...
def initialScan():
	...
	portList.append("3050")
	portList.append("3306")
	portList.append("5432")
	portList.append("8080")
	for port in portList:
		"propecia "+ipRange+" "+port

def generateCommands():
	os.chdir(origPath)
	print "- Generating nmap commands"
	print "- Running nmap against the below hosts"
	with open(filename) as f:
		hostNo = ""
	        for line in f:
        	        hostNo = line.split(":")[0]
			...
			if len(line.strip())>0:
				print line
				if ":" not in line:
					print "Symbol ':' is missing from "+line
					sys.exit()
        	        	...
        	        	if len(tcpportNo)>0 and len(udpportNo)>0:
        	                	arg = "sudo nmap -Pn -T4 -sT -sU -n -sV  -A  -v --open --script default -p T:"+tcpportNo.strip()+",U:"+udpportNo.strip()+" -oA nmap_"+str(hostNo)
        	                	cmd = arg+" "+hostNo
			        	scanTCPList.append((hostNo,cmd))
                		if len(tcpportNo)>0 and len(udpportNo)<1:
                       		 	arg = "sudo nmap -Pn -T4 -sT -sU -n -sV  -A  -v --open --script default -p T:"+tcpportNo+" -oA nmap_"+str(hostNo)
                        		cmd = arg+" "+hostNo
		        		scanTCPList.append((hostNo,cmd))
                		if len(tcpportNo)<1 and len(udpportNo)>0:
                        		arg = "sudo nmap -Pn -T4 -sT -sU -n -sV  -A  -v --open --script default -p U:"+udpportNo+" -oA nmap_"+str(hostNo)
                        		cmd = arg+" "+hostNo
		        		scanTCPList.append((hostNo,cmd))
	...
	for x in results:
		...
		for i in x:	
			if "Nmap scan report for " in i:
				...
				portDescription = (portStatus[1].strip()).replace(portType,"").strip()
